...
GMSR_MSCI = np.float64(3_592 * 1_000_000)

# Import Global Percentage
CN_Target_Percentage = float(os.getenv("CN_Target_Percentage"))
current_datetime = os.getenv("current_datetime")
GMSR_Lower_Buffer_AllCap = float(os.getenv("GMSR_Lower_Buffer"))
GMSR_Upper_Buffer_AllCap = float(os.getenv("GMSR_Upper_Buffer"))
...
# Set a Threshold for Target_Turnover_Reduction
Target_Turnover_Reduction = 0.85
...
AllCapIndexOutput = pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" + current_datetime + ".csv").select(
    pl.col(["Date", "Internal_Number", "ENTITY_QID"])
).with_columns(pl.col("Date").cast(pl.Date))

# Read again AllCapIndexOuput but keeping all the needed columns
AllCapIndex = (pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" 
                    + current_datetime + ".csv").select(
                    pl.col(["Date", "Internal_Number", "ENTITY_QID", "Instrument_Name", "Country"])
                    ).with_columns(pl.col("Date").cast(pl.Date)))

# Read again AllCapIndexOuput but keeping all the needed columns for Lite Review
AllCapIndexLiteReview = (pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" 
                    + current_datetime + ".csv").with_columns(pl.col("Date").cast(pl.Date))).select(
                        pl.col(["Date", "Internal_Number", "Instrument_Name", "ENTITY_QID", "Country", "Size", "Shadow_Company"]))
...
##################################
## Turnover Artifical Reduction ##
##################################
def Turnover_Artifical_Reduction(TopPercentage_Securities, Standard_Index, Screened_Securities, date, Pivot_TOR, Target_Turnover_Reduction):

    # List of Unique Dates
    Dates_List = Pivot_TOR.index.to_list()
    IDX_Current = Dates_List.index(date.strftime("%Y-%m-%d"))
    # Calculate the Previous_Date given the current review date
    Previous_Date = datetime.datetime.strptime(Dates_List[max(0, IDX_Current - 1)], "%Y-%m-%d").date()
            
    # Mark the Securities that are new Additions
    TopPercentage_Securities = TopPercentage_Securities.with_columns(
        ~pl.col("Internal_Number").is_in(Standard_Index.filter(pl.col("Date")==Previous_Date).select(pl.col("Internal_Number"))).alias("Addition")
    )

    # Find the Securities which have been deleted (only Non-Shadow)
    Deleted_Securities = (Standard_Index.filter((pl.col("Date")==Previous_Date) & (pl.col("Country")==country) & 
                                                (pl.col("Shadow_Company")==False)).
                          filter(~pl.col("Internal_Number").is_in(TopPercentage_Securities.select(pl.col("Internal_Number"))))).select(
                              pl.col("Internal_Number")
                          )
    
    # Check if the Deleted_Securities were excluded due to failing Screens and keep only those
    Deleted_Securities = Deleted_Securities.with_columns(
                            pl.col("Internal_Number").is_in(
                                Screened_Securities.filter(pl.col("Date") == date)
                                                .select(pl.col("Internal_Number"))
                            ).alias("Screens_Passed")
                        ).filter(pl.col("Screens_Passed")==True)
    
    # Count the number of Deletions
    if Deleted_Securities.height > 1:

        # Add current Full_MCAP_USD_Cutoff to Deleted_Securities
        Deleted_Securities = (Deleted_Securities.with_columns(
        ...
        # Calculate Full_MCAP_USD_Cutoff
        Deleted_Securities = Deleted_Securities.with_columns(
        ...
        # Sort Deleted_Securities by Full_MCAP_USD_Cutoff
        Deleted_Securities = (Deleted_Securities.select(pl.col(["Date", "Internal_Number", "Full_MCAP_USD_Cutoff"]))
                              .sort("Full_MCAP_USD_Cutoff", descending=True))
        
        # Calculate the Cum_Weight of Deletions
        Deleted_Securities = (
            Deleted_Securities.with_columns(
                pl.col("Full_MCAP_USD_Cutoff").cum_sum().alias("Cum_Full_MCAP_USD_Cutoff")
            )
            .with_columns(
                (pl.col("Cum_Full_MCAP_USD_Cutoff") / pl.col("Cum_Full_MCAP_USD_Cutoff").max())
                .alias("Cum_Weight")
            )
        )

        # Select only the Securities lower than Target_Turnover_Reduction and the first one crossing it
        Deleted_Securities = Deleted_Securities.filter(pl.col("Cum_Weight") < Target_Turnover_Reduction).vstack(
                            Deleted_Securities.filter(pl.col("Cum_Weight") >= Target_Turnover_Reduction).head(1)
        )

        # Add back the information to be added into TopPercentage_Securities
        Deleted_Securities = Deleted_Securities.join(Emerging.select(pl.col(["Date", "Instrument_Name", "Internal_Number",
                            "ENTITY_QID", "Country", "Free_Float_MCAP_USD_Cutoff"])),
                            on=["Date", "Internal_Number"], how="left").drop("Cum_Full_MCAP_USD_Cutoff", "Cum_Weight").with_columns(
                                [pl.lit("Standard").alias("Size"),
                                 pl.lit(False).alias("Shadow_Company"),
                                 pl.lit("Buffer").alias("Case")]
                            )
        
        # Remove X number of additions where X is the number of Deletions that have been reintroduced to the Index
        Frame_Mask = (
            TopPercentage_Securities
            .filter(pl.col("Addition") == True)  # Filter rows where Addition is True
            .sort("Full_MCAP_USD_Cutoff")        # Sort by Full_MCAP_USD_Cutoff
            .head(Deleted_Securities.height)     # Select the smallest X rows
            .select(pl.col("Internal_Number"))   # Extract their Internal_Numbers
        )

        # Drop the column Addition from TopPercentage_Securities
        TopPercentage_Securities = TopPercentage_Securities.drop("Addition")

        # Check if number of Deletions and Additions are matching
        if Deleted_Securities.height == Frame_Mask.height:
            
            # Apply the mask to exclude the selected rows
            TopPercentage_Securities = TopPercentage_Securities.filter(
                ~pl.col("Internal_Number").is_in(Frame_Mask)
            )

            # Add the Deletions back
            TopPercentage_Securities = TopPercentage_Securities.vstack(Deleted_Securities.select(TopPercentage_Securities.columns)) 

        else:

            # In case the number of Deletions is higher than the number of Additions
            Deleted_Securities = Deleted_Securities.sort("Full_MCAP_USD_Cutoff", descending=True).head(Frame_Mask.height)
...
def FOR_Sreening(Screened_Securities, Frame: pl.DataFrame, Full_Frame: pl.DataFrame, Pivot_TOR, Standard_Index, Small_Index, date, Segment: pl.Utf8) -> pl.DataFrame:
    ...
    for country in Frame.select(["Country"]).unique().sort("Country").to_series():
        ...
        Temp_Screened_Securities = Screened_Securities.filter(pl.col("Date")==Previous_Date)
...
#######Quarterly Turnover#########
##################################
def Turnover_Check(Frame: pl.DataFrame, Pivot_TOR: pl.DataFrame, Threshold_NEW, Threshold_OLD, date, Starting_Date, Segment: pl.Utf8, Screened_Securities) -> pl.DataFrame:
        ...
        if date == Starting_Date:
            ...
            if Segment == "Emerging":
                Frame = Frame.with_columns(
                                            pl.when(
                                                pl.col("Internal_Number").is_in(
                                                    AllCapIndexOutput.filter(pl.col("Date") == Previous_Date).select(pl.col("Internal_Number"))
                                                )
                                            )
...
##########12M Turnover############
##################################
def Turnover_Check_12M(Frame: pl.DataFrame, Pivot_TOR_12M: pl.DataFrame, Threshold_NEW_12M, Threshold_OLD_12M, date, Starting_Date, Segment: pl.Utf8, Screened_Securities) -> pl.DataFrame:
...
def Minimum_FreeFloat_Country(TopPercentage, temp_Country, Lower_GMSR, Upper_GMSR, date, country, Segment: pl.Utf8, Original_MCAP_Emerging):
    ...
    if (date == Starting_Date) | (len(Output_Standard_Index.filter(pl.col("Country") == country)) == 0):
        ...
        if ((date.month == 6 or date.month == 12)) and (date < datetime.date(2023,3,20)):
            ...
            # Avoid introduction of new Internal_Number given by duplicate ENTITY_QID
            TopPercentage_Securities = TopPercentage_Securities.with_columns(
                                    ~pl.col("Internal_Number").is_in(
                                        AllCapIndexLiteReview
                                        .filter((pl.col("Country") == country) & (pl.col("Date") == Previous_Date))
                                        .select(pl.col("Internal_Number"))
                                    ).alias("ToBeExcluded")
                                ).filter(pl.col("ToBeExcluded") == False).drop("ToBeExcluded")
            
            # Verify if there were additions compared to the Previous Standard Index at Security Level
            TopPercentage_Securities = TopPercentage_Securities.with_columns(pl.col("Internal_Number").is_in(
                    Standard_Index.filter((pl.col("Country")==country) & (pl.col("Date")==Previous_Date)).
                    select(pl.col("Internal_Number"))).alias("PREV_STANDARD"))
            ...
            # Verify for Small Securities which made it into Standard Index
            if TopPercentage_Securities.filter(pl.col("PREV_STANDARD") == False).height > 0:

                TopPercentage_Securities = TopPercentage_Securities.with_columns(
                    pl.when(pl.col("Shadow_Company").is_null())
                    .then(
                        pl.when(pl.col("Free_Float_MCAP_USD_Cutoff") > (Country_Cutoff / 2))
                        .then(False)  # Set to FALSE if condition is met
                        .otherwise(True)  # Set to TRUE otherwise
                    )
                    .otherwise(pl.col("Shadow_Company"))  # Keep the original value if not null
                    .alias("Shadow_Company")  # Overwrite the column
                )
            ...
            # Lastly drop column PREV_STANDARD
            TopPercentage_Securities = TopPercentage_Securities.drop("PREV_STANDARD")

            # Keep only needed columns
            TopPercentage = TopPercentage.select(pl.col(["Date", "ENTITY_QID", "Country", "Size", "Case"]))
            TopPercentage_Securities = TopPercentage_Securities.select(pl.col(["Date", "Internal_Number", "Instrument_Name", "ENTITY_QID",
                                                                               "Country", "Free_Float_MCAP_USD_Cutoff", "Full_MCAP_USD_Cutoff",
                                                                               "Size", "Shadow_Company"]))
...
def Index_Rebalancing_Box(Frame: pl.DataFrame, SW_ACALLCAP, Output_Count_Standard_Index, Lower_GMSR, Upper_GMSR, country, date, Excel_Recap,  Right_Limit, Left_Limit, Segment: pl.Utf8, writer):
    ...
    def ChairsSorting(AllCapIndexOutput, Output_Standard_Index, Securities_Cutoff, country, Previous_Date, date, FX_Cutoff):

        # Filter for Country and Previous_Date Review at Security_Level #
        # AllCapIndex from Previous Quarter #        
        Starting_Universe = AllCapIndex.filter((pl.col("Country") == country) & (pl.col("Date") == Previous_Date)).select(["Date", "Internal_Number", 
    ...
    PositionFullMCAP = ChairsSorting(AllCapIndexOutput, Output_Standard_Index, Securities_Cutoff, country, Previous_Date, date, FX_Cutoff)
    ...
    # Recalculate Analysis Variables
    Last_FullMcap = TopPercentage.tail(1).select("Full_MCAP_USD_Cutoff_Company").to_numpy()[0][0]
    Last_CumWeight = TopPercentage.tail(1).select("CumWeight_Cutoff").to_numpy()[0][0]
...
Trading_Days = pl.read_parquet(r"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\TMI_Based\Trading Days\Trading_Days.parquet").filter(
...
NumberOfChairs = pl.DataFrame({
            "Date": pl.Series([], dtype=pl.Date),
            "Country": pl.Series([], dtype=pl.Utf8),
            "Chairs": pl.Series([], dtype=pl.Int64)
})
...
with pd.ExcelWriter(Output_File, engine='xlsxwriter') as writer:
    for date in Emerging.select(["Date"]).unique().sort("Date").to_series():
        ...
        if date == Starting_Date: 
            ...
            Developed_Screened_12M = Turnover_Check_12M(temp_Developed, Pivot_TOR_12M, Threshold_NEW_12M, Threshold_OLD_12M, date, Starting_Date, "Developed", Screened_Securities)
            Emerging_Screened_12M = Turnover_Check_12M(temp_Emerging, Pivot_TOR_12M, Threshold_NEW_12M, Threshold_OLD_12M, date, Starting_Date, "Emerging", Screened_Securities)
            ...
            # Apply the Check on Turnover for all Components
            Developed_Screened = Turnover_Check(temp_Developed, Pivot_TOR, Threshold_NEW, Threshold_OLD, date, Starting_Date, "Developed", Screened_Securities)
            Emerging_Screened = Turnover_Check(temp_Emerging, Pivot_TOR, Threshold_NEW, Threshold_OLD, date, Starting_Date, "Emerging", Screened_Securities)
            ...
            for country in temp_Emerging_Aggregate.select(pl.col("Country")).unique().sort("Country").to_series():
                ...
                # Add information of Number of Chairs
                ChairsRow = pl.DataFrame({
                    "Date": [date],
                    "Country": [country],
                    "Chairs": [len(TopPercentage)]
                })

                # Stack the Results
                NumberOfChairs = NumberOfChairs.vstack(ChairsRow) 
            ...
            for country in temp_Emerging_Aggregate.select(pl.col("Country")).unique().sort("Country").to_series():
                ...
                if len(Output_Count_Standard_Index.filter((pl.col("Country") == country) & (pl.col("Date") == Previous_Date))) > 0:
                    ...
                    # Lite Review for Jun and December
                        ...
                        Previous_Composition = AllCapIndexLiteReview.filter((pl.col("Country") == country) & (pl.col("Date") == Previous_Date)).drop("Shadow_Company")
                ...
                # TopPercentage_Securities = Turnover_Artifical_Reduction(TopPercentage_Securities, 
                #                                                         Standard_Index, Screened_Securities, date, 
                #                                                         Pivot_TOR, Target_Turnover_Reduction)
...
# Store the Results
Standard_Index.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\Standard_Index_Security_Level_CNTarget_{CN_Target_Percentage}_" + current_datetime + ".csv")
Standard_Index_Shadows.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\Standard_Index_Security_Level_Shadows_CNTarget_{CN_Target_Percentage}_" + current_datetime + ".csv")
NumberOfChairs = NumberOfChairs.pivot(
    values="Chairs",  # The column to use for cell values
    index="Country",  # The column to use for row labels
    columns="Date"    # The column to use for column labels
).write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\Standard_Chairs_Recap_{CN_Target_Percentage}_" + current_datetime + ".csv")
...
GMSR_Frame.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\GMSR_Frame_STANDARD.csv")
# EMS_Frame.write_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Standard\EMS_Frame_{Percentage}_{Coverage_Adjustment}_" + current_datetime + ".csv")
