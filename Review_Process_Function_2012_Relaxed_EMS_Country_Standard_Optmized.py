...
# AllCap Index Output
AllCapIndexOutput = pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" + current_datetime + ".csv").select(
    pl.col(["Date", "Internal_Number", "ENTITY_QID"])
).with_columns(pl.col("Date").cast(pl.Date))

# Read again AllCapIndexOuput but keeping all the needed columns
AllCapIndex = (pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" 
...
AllCapIndexLiteReview = (pl.read_csv(rf"C:\Users\lbabbi\OneDrive - ISS\Desktop\Projects\SAMCO\TMI_Based\Output\Tests\AllCap_Index_Security_Level_Shadows_{GMSR_Upper_Buffer_AllCap}_{GMSR_Lower_Buffer_AllCap}_" 
...
def Equity_Minimum_Size(temp_TMI, df: pl.DataFrame, Pivot_TOR, EMS_Frame, date, Segment: pl.Utf8, Screened_Securities, temp_Exchanges_Securities, FullListSecurities) -> pl.DataFrame:
    ...
    if Segment == "Developed":
        ...
        if date != Starting_Date:
            ...
            Security_Level_DF = Security_Level_DF.with_columns(
            # Create a mask column that applies the screen only i4f InPrevScreened_Universe is FALSE
                pl.when(
                    (pl.col("InPrevScreened_Universe") == False) &
