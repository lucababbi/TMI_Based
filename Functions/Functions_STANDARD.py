...
#########FOR Screening############
##################################
def FOR_Screening(Screened_Securities, Frame: pl.DataFrame, Full_Frame: pl.DataFrame, Pivot_TOR, Standard_Index, Small_Index, date, Segment: pl.Utf8) -> pl.DataFrame:
...
#######Equity Minimum Size########
##################################
def Equity_Minimum_Size(temp_TMI, df: pl.DataFrame, Pivot_TOR, EMS_Frame, date, Segment: pl.Utf8, Screened_Securities, temp_Exchanges_Securities, Entity_ID, Starting_Date, MSCI_Equity_Minimum_Size) -> pl.DataFrame:
...
##################################
def Minimum_FreeFloat_Country(TopPercentage, temp_Country, Lower_GMSR, Upper_GMSR, date, country, Segment: pl.Utf8, Original_MCAP_Emerging, LIF_Stored, Starting_Date, Output_Standard_Index, temp_Emerging, ETF,
                              Pivot_TOR, Standard_Index, AllCap_Index_Shadow, Original_MCAP_Developed):
...
def Index_Creation_Box(Frame: pl.DataFrame, Lower_GMSR, Upper_GMSR, country, date, Percentage, Right_Limit, Left_Limit, Segment: pl.Utf8):
    ...
    # No Country_Adjustment as we are building Standard
    ...
    .filter(pl.col("CumWeight_Cutoff") <= Country_Percentage)
    .vstack(
        temp_Country
        .select([
            "Date", 
...
########Index Rebalancing#########
##################################
def Index_Rebalancing_Box(Frame: pl.DataFrame, SW_ACALLCAP, Output_Count_Standard_Index, Lower_GMSR, Upper_GMSR, country, date, Right_Limit, Left_Limit, Segment: pl.Utf8, Pivot_TOR, Output_Standard_Index, 
                          Emerging, AllCap_Index_Shadow, Securities_Cutoff, FX_Cutoff, GMSR_Frame_STANDARD):
    temp_Country = Frame.filter((pl.col("Date") == date) & (pl.col("Country") == country))
    ...
    def ChairsSorting(AllCap_Index_Shadow, Output_Standard_Index, Securities_Cutoff, country, Previous_Date, date, FX_Cutoff):

        # Filter for Country and Previous_Date Review at Security_Level #
        # AllCapIndex from Previous Quarter #        
        Starting_Universe = AllCap_Index_Shadow.filter((pl.col("Country") == country) & (pl.col("Date") == Previous_Date)).select(["Date", "Internal_Number", 
    ...
    PositionFullMCAP = ChairsSorting(AllCap_Index_Shadow, Output_Standard_Index, Securities_Cutoff, country, Previous_Date, date, FX_Cutoff)
    ...
    Lower_Proximity_Bound = (Lower_GMSR, GMSR_Frame_STANDARD.select(["GMSR_Emerging", "Date"]).filter(pl.col("Date") == date).to_numpy()[0][0] * 0.575)
    Upper_Proximity_Bound = (GMSR_Frame_STANDARD.select(["GMSR_Emerging", "Date"]).filter(pl.col("Date") == date).to_numpy()[0][0], Upper_GMSR)
